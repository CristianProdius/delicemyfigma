...
const GalleryItem = ({ item, index }: GalleryItemProps) => {
  ...
  const getHeightClass = (size: string) => {
    switch (size) {
      case "tall":
        // Account for 2 rows + gap between them
        return "h-[516px] sm:h-[624px] lg:h-[632px]";
      case "wide":
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        className="relative w-full h-full overflow-hidden rounded-2xl sm:rounded-3xl shadow-lg hover:shadow-2xl transition-shadow duration-500"
      >
        {/* Background Image */}
        ...
        <AnimatePresence>
          {isHovered && (
            <motion.div
              ...
              className="absolute inset-0 p-6 sm:p-8 flex flex-col justify-between z-10"
            >
              {/* Top Actions */}
              ...
              <div>
                ...
                >
                  <h3 className="text-2xl sm:text-3xl font-light text-white mb-2 [font-family:var(--font-playfair)]">
                    {item.title}
                  </h3>
                  <p className="text-sm sm:text-base text-white/80 mb-4 [font-family:var(--font-inter)] font-light">
                    {item.description}
                  </p>
                </motion.div>
        ...
        <div className="absolute top-4 left-4 z-20">
          <motion.div
            ...
            className="px-3 py-1 rounded-full bg-white/90 backdrop-blur-sm text-[#451C15] text-xs sm:text-sm font-medium capitalize [font-family:var(--font-inter)]"
          >
            {item.category}
          </motion.div>
        </div>
...
export const PortfolioSection = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative w-full py-20 sm:py-24 bg-gradient-to-b from-white via-[#FFF9F5] to-white overflow-hidden"
    >
      ...
      <div className="relative z-10 px-4 sm:px-6 lg:px-8 max-w-[95%] xl:max-w-[90%] mx-auto">
        ...
        >
          <span className="inline-block text-sm sm:text-base text-[#451C15]/70 tracking-[0.3em] uppercase font-medium mb-4 [font-family:var(--font-inter)]">
            {portfolioContent.title}
          </span>
          <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl xl:text-8xl font-light text-[#451C15] mb-6 [font-family:var(--font-playfair)]">
            {portfolioContent.subtitle}
          </h2>
          <p className="text-base sm:text-lg lg:text-xl text-[#451C15]/60 max-w-2xl mx-auto font-light [font-family:var(--font-inter)]">
        ...
        >
          {portfolioContent.categories.map((category, index) => (
            <motion.button
              ...
              className={`px-5 sm:px-6 py-2.5 sm:py-3 rounded-full transition-all duration-300 text-sm sm:text-base font-light [font-family:var(--font-inter)] ${
        ...
        <motion.div
          layout
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 auto-rows-[250px] sm:auto-rows-[300px]"
        >
        ...
        >
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="group relative px-10 sm:px-12 py-4 sm:py-5 bg-[#451C15] text-[#E0D9C9] rounded-full overflow-hidden transition-all duration-300 hover:shadow-2xl text-base sm:text-lg font-light [font-family:var(--font-inter)]"
          >
